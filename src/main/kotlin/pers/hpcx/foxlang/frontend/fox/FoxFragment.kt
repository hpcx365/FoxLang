...
fun String.toFoxFragments() = FoxSourceFragmentationContext(this).run()

private class FoxSourceFragmentationContext(
    private val text: String,
) {
    
    private val lines = text.lines()
    private val errors = mutableListOf<String>()
    private val fragments = mutableListOf<FoxFragment>()
    
    private var line = 0
    private var column = 0
    
    fun run(): SourceFragmentationResult {
        return scanFragments()
            ?.let { SourceFragmentationSuccess(Source(text, it)) }
            ?: SourceFragmentationFailure(errors)
    }
    
    private fun move(numChars: Int) {
        ...
        if (newColumn < lineText.length) {
            ...
            if (line + 1 < lines.size && fragments.isNotEmpty() && fragments.last() !is LineBreakFragment) {
                fragments += lineBreak(line, lineText.length)
            }
            line++
            column = 0
        }
    ...
    private fun scanFragments(): List<FoxFragment>? {
        while (line < lines.size) {
            ...
            if (lines[line].startsWith("rf\"", column)) {
                val end = scanFormattedString(line, column, isRaw = true) ?: return null
                move(end - column)
                continue
            }
            
            if (lines[line].startsWith("f\"", column)) {
                val end = scanFormattedString(line, column, isRaw = false) ?: return null
                move(end - column)
                continue
            }
            ...
            if (lines[line].startsWith("r\"", column)) {
                ...
                fragments += rawStringLiteral(line, start, index + 1)
                move(index - column + 1)
                continue
            }
            ...
            if (lines[line][column] == '\'') {
                ...
                fragments += charLiteral(line, start, index + 1) ?: return null
                move(index - column + 1)
                continue
            }
            ...
            if (lines[line][column] == '"') {
                ...
                fragments += stringLiteral(line, start, index + 1) ?: return null
                move(index - column + 1)
                continue
            }
            ...
            if (column < end) {
                fragments += plain(line, column, end)
                move(end - column)
                continue
            }
            
            fragments += plain(line, column, column + 1)
            move(1)
        }
    ...
    private fun <T> recordError(message: String): T? {
        errors += message
        return null
    }
    
    private fun plain(line: Int, start: Int, end: Int): PlainFragment {
        val lineText = lines[line]
        val text = lineText.substring(start, end)
        return PlainFragment(line, start, text)
    }
    
    private fun lineBreak(line: Int, start: Int): LineBreakFragment {
        return LineBreakFragment(line, start)
    }
    
    private fun charLiteral(line: Int, start: Int, end: Int): CharLiteralFragment? {
    ...
    private fun stringLiteral(line: Int, start: Int, end: Int): StringLiteralFragment? {
        var i = 0
    ...
    private fun rawStringLiteral(line: Int, start: Int, end: Int): StringLiteralFragment {
        var i = 0
        val builder = StringBuilder()
    ...
    private fun scanFormattedString(line: Int, start: Int, isRaw: Boolean): Int? {
        val lineText = lines[line]
        ...
        fun flushText(end: Int) {
            if (builder.isNotEmpty()) {
                fragments += FormattedStringTextFragment(line, textColumn, builder.toString())
                builder.clear()
            }
            textColumn = end
        }
        
        fragments += FormattedStringStartFragment(line, start, isRaw)
        
        while (index < lineText.length) {
            when (val char = lineText[index]) {
                '"' -> {
                    flushText(index)
                    fragments += FormattedStringEndFragment(line, index)
                    return index + 1
                }
                
                '{' -> {
                    flushText(index)
                    fragments += FormattedExpressionStartFragment(line, index)
    ...
    private fun scanFormattedExpression(line: Int, start: Int): Int? {
        val lineText = lines[line]
        var index = start
        var braceDepth = 0
        ...
        while (index < lineText.length) {
            ...
            if (lineText.startsWith("r\"", index)) {
                ...
                fragments += rawStringLiteral(line, index, end + 1)
                index = end + 1
                continue
            }
            ...
            if (lineText[index] == '\'') {
                ...
                fragments += charLiteral(line, index, end + 1) ?: return null
                index = end + 1
                continue
            }
            ...
            if (lineText[index] == '"') {
                ...
                fragments += stringLiteral(line, index, end + 1) ?: return null
                index = end + 1
                continue
            }
            
            if (lineText[index] == '{') {
                fragments += PlainFragment(line, index, "{")
                braceDepth++
                index++
                continue
            }
            ...
            if (lineText[index] == '}') {
                if (braceDepth == 0) {
                    fragments += FormattedExpressionEndFragment(line, index)
                    return index + 1
                }
                fragments += PlainFragment(line, index, "}")
                braceDepth--
            ...
            if (index < end) {
                fragments += plain(line, index, end)
                index = end
                continue
            }
            
            fragments += plain(line, index, index + 1)
            index++
        }
    ...
    private fun parseUnicodeEscape(lineText: String, line: Int, column: Int): Char? {
